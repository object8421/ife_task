...
class Tank_auth
{
	 ...
	 * @return	bool
	 */
	function login($login, $password, $remember, $lang, $login_by_username, $login_by_email)
	{
				...
				if ($hasher->CheckPassword($password, $user->password)) {		// password ok
					...
					if ($user->banned == 1) {									// fail - banned
						...
						if ($user->activated == 0) {							// fail - not activated
							...
							if ($lang != '') {
								$this->ci->users->set_language($user->id, $lang);   // update user language
							}
							if ($remember) {
								if ($lang != '') {
									$this->create_autologin($user->id, $lang);
								} else {
									$this->create_autologin($user->id, $user->lang);
								}
							}
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation, $lang, $description)
	{
	...
	function change_password2($id, $new_pass)
	{
		//$user_id = $this->ci->session->userdata('user_id');
	
		if (!is_null($user = $this->ci->users->get_user_by_id($id, TRUE))) {
	 ...
	 * @return	bool
	 */
	private function create_autologin($user_id, $lang)
	{
		...
		if ($this->ci->user_autologin->set($user_id, md5($key))) {
			...
			set_cookie(array(
				'name' 		=> $this->ci->config->item('lang_cookie_name', 'tank_auth'),
				'value'		=> $lang,
				'expire'	=> $this->ci->config->item('lang_cookie_life', 'tank_auth'),
			));
	...
	{
		$users = $this->ci->users->get_user_by_id($user_id, 1);
		if (is_null($users)) {
			return "";
		} else {
